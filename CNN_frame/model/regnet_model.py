...
class Bottleneck(nn.Module):
    def __init__(self,
        ...
        self.conv1 = ConvBNAct(in_chans=in_chans, out_c=out_c, kernel_s=1)
        self.conv2 = ConvBNAct(in_chans=out_c,
                               out_c=out_c,
                               kernel_s=3,
                               stride=stride,
                               padding=1,
        ...
        if se_ratio > 0:
            self.se = SqueezeExcitation(in_chans, out_c, se_ratio)
        else:
            self.se = nn.Identity()

        self.conv3 = ConvBNAct(in_chans=out_c, out_c=out_c, kernel_s=1, act=None)
        ...
        if (in_chans != out_c) or (stride != 1):
            self.downsample = ConvBNAct(in_chans=in_chans, out_c=out_c, kernel_s=1, stride=stride, act=None)
        else:
...
class RegStage(nn.Module):
    def __init__(self,
        ...
        for i in range(depth):
            ...
            block_in_c = in_chans if i == 0 else out_c

            name = "b{}".format(i + 1)
            self.add_module(name,
                            Bottleneck(in_chans=block_in_c,
                                       out_c=out_c,
                                       stride=block_stride,
...
class RegNet(nn.Module):
    ...
    def __init__(self,
        ...
        self.stem = ConvBNAct(in_chans, out_c=stem_c, kernel_s=3, stride=2, padding=1)
        ...
        for i, stage_args in enumerate(stage_info):
            ...
            self.add_module(stage_name, RegStage(in_chans=input_channels, **stage_args))
...
def create_regnet(model_name="RegNetX_200MF", in_channels=4, num_classes=1000):
    ...
    model = RegNet(cfg=model_cfgs[model_name], in_chans=in_channels,num_classes=num_classes)
    return model
